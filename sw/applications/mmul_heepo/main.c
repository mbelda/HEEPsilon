...
void main()
{
  ...
  if(ROWS_A < CGRA_N_COLS || COLS_B < CGRA_N_ROWS){
    ...
    printf("Loading CGRA kernel...\n");
    reset_csr_counters();
    load_kernel();
    read_csr_counters();

    printf("Preparing CGRA input...\n");
    reset_csr_counters();
    ...
    // CGRA Execution
    read_csr_counters();
    printf("Starting CGRA execution...\n");
    reset_csr_counters();
    cgra_intr_flag = 0;
    ...
    printf("Processing extra rows/cols for non multiple dimensions...\n");
    reset_csr_counters();
    ...
    reset_csr_counters();
    mmulSoftware(cpu_out);
    read_csr_counters();

    checkErrors(matrixC, cpu_out);
...
void checkErrors(int32_t * outcgra, int32_t * res){
  int errors = 0;
  //printf("CGRA res: [");
