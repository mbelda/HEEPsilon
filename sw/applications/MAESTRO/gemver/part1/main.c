...
void main()
{
  ...
  printf("Running gemver part1 for size %d...", N);
...
*/
  ...
  uint32_t first_addr_u1 = (uint32_t)&u1[0];
  uint32_t first_addr_u2 = (uint32_t)&u2[0];
  uint32_t first_addr_v1 = (uint32_t)&v1[0];
  uint32_t first_addr_v2 = (uint32_t)&v2[0];

  // Col 0
  cgra_input[0][0] = first_addr_A;
  cgra_input[0][1] = first_addr_u2 + (1 * 4);
  cgra_input[0][2] = first_addr_A + (2 * N * 4);
  cgra_input[0][3] = first_addr_u1 + (3 * 4);
  cgra_input[0][4] = first_addr_v2;
  cgra_input[0][5] = loopIit;
  cgra_input[0][6] = first_addr_v1;
  cgra_input[0][7] = 0;   // Padding si es necesario

  // Col 1
  cgra_input[1][0] = first_addr_u1;
  cgra_input[1][1] = first_addr_A + (1 * N * 4) + (1 * 4);
  cgra_input[1][2] = first_addr_u2 + (2 * 4);
  cgra_input[1][3] = first_addr_A + (3 * N * 4) + (1 * 4);
  cgra_input[1][4] = N;
  cgra_input[1][5] = first_addr_v2 + (1 * 4);
  cgra_input[1][6] = N;
  cgra_input[1][7] = first_addr_v1 + (1 * 4);

  // Col 2
  cgra_input[2][0] = first_addr_A + (2 * 4);
  cgra_input[2][1] = first_addr_u1 + (1 * 4);
  cgra_input[2][2] = first_addr_A + (2 * N * 4) + (2 * 4);
  cgra_input[2][3] = first_addr_u2 + (3 * 4);
  cgra_input[2][4] = first_addr_v1 + (2 * 4);
  cgra_input[2][5] = loopJit;
  cgra_input[2][6] = first_addr_v2 + (2 * 4);
  cgra_input[2][7] = 0;   // Padding si es necesario

  // Col 3
  cgra_input[3][0] = first_addr_u2;
  cgra_input[3][1] = first_addr_A + (1 * N * 4) + (3 * 4);
  cgra_input[3][2] = first_addr_u1 + (2 * 4);
  cgra_input[3][3] = first_addr_A + (3 * N * 4) + (3 * 4);
  cgra_input[3][4] = N;
  cgra_input[3][5] = first_addr_v1 + (3 * 4);
  cgra_input[3][6] = N;
  cgra_input[3][7] = first_addr_v2 + (3 * 4);
  ...
  // CGRA Execution
  cgra_intr_flag = 0;
  cgra_set_kernel( &cgra, cgra_slot, PART1_KERNEL_ID);
