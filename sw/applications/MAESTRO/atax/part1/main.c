...
* @file   main.c
* @date   08/07/2026
* @brief  An application to run ATAX Part 1 (tmp = A * x) on CGRA.
*
*/
...
void main()
{
  ...
  printf("Running atax part1 for size M=%d, N=%d...", M, N);
...
*/
  ...
  uint32_t first_addr_tmp = (uint32_t)&tmp[0]; // Destino Parte 1
  ...
  cgra_input[0][7] = first_addr_tmp + (12 * 4);
  ...
  cgra_input[1][4] = first_addr_tmp + (1 * 4);
  cgra_input[1][5] = N;
  ...
  cgra_input[2][5] = N;
  cgra_input[2][6] = first_addr_tmp + (10 * 4);
  ...
  cgra_input[3][4] = first_addr_tmp + (7 * 4);
  cgra_input[3][5] = N;
  ...
  // CGRA Execution
  cgra_intr_flag = 0;
  cgra_set_kernel( &cgra, cgra_slot, ATAX_P1_KERNEL_ID);
