...
* @file   main.c
* @date   08/07/2026
* @brief  An application to run ATAX Part 2 (y = A^T * tmp + y) on CGRA.
*
*/
...
// CGRA input buffers (Aumentado a 8 debido al tamaño de las listas de configuración)
...
void main()
{
  ...
  printf("Running atax part2 for size M=%d, N=%d...", M, N);
...
*/
  ...
  uint32_t first_addr_tmp = (uint32_t)&tmp[0];
  uint32_t first_addr_y   = (uint32_t)&y[0]; // Destino final Parte 2

  // Col 0
  cgra_input[0][0] = first_addr_A;
  cgra_input[0][1] = first_addr_y + (4 * 4);
  cgra_input[0][2] = M;
  cgra_input[0][3] = N;
  ...
  cgra_input[1][3] = first_addr_y + (13 * 4);
  cgra_input[1][4] = loopJit;
  cgra_input[1][5] = loopIit;
  cgra_input[1][6] = first_addr_tmp;

  // Col 2
  cgra_input[2][0] = first_addr_y + (2 * 4);
  ...
  cgra_input[3][0] = M;
  cgra_input[3][1] = N;
  cgra_input[3][2] = first_addr_y + (11 * 4);
  cgra_input[3][3] = first_addr_A + (15 * 4);
  cgra_input[3][4] = first_addr_tmp;
  ...
  // CGRA Execution
  cgra_intr_flag = 0;
  cgra_set_kernel( &cgra, cgra_slot, ATAX_P2_KERNEL_ID);
