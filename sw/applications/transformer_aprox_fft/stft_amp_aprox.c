...
// Function to compute the logarithm of the amplitude
int32_t compute_log_amp_fp(int32_t real, int32_t imag)
{
...
int32_t compute_log_amp_fxp_lut(int32_t real, int32_t imag) {
    ...
    if (sqmag == 0) {
        return (int32_t)(-23 * (1 << NUM_FRACTION_BITS)); // Q12, -94208
    }
...
int32_t compute_log_amp_fxp_approx(int32_t real, int32_t imag) {
    real = MUL_HQ(real, 25) >> (NUM_FRACTION_BITS - 9);
    imag = MUL_HQ(imag, 25) >> (NUM_FRACTION_BITS - 9);
    int32_t real2 = MUL_LONG(real, real) >> NUM_FRACTION_BITS;
    int32_t imag2 = MUL_LONG(imag, imag) >> NUM_FRACTION_BITS;
    ...
    int32_t log_approx = (exponent << NUM_FRACTION_BITS) + (mantissa >> (31 - NUM_FRACTION_BITS));

    return log_approx;
}

// Function to compute the magnitude of the complex number
int32_t compute_magnitude_fxp(int32_t real, int32_t imag) {
    real = MUL_HQ(real, 25) >> (NUM_FRACTION_BITS - 9);
    imag = MUL_HQ(imag, 25) >> (NUM_FRACTION_BITS - 9);
...
int32_t compute_magnitude_fxp_opt(int32_t real, int32_t imag) {
