...
static int32_t __attribute__((section(".xheep_data_interleaved"))) matrixA[(ROWS_A+1)*COLS_A];
static int32_t __attribute__((section(".xheep_data_interleaved"))) matrixB[COLS_A*COLS_B];
static int32_t __attribute__((section(".xheep_data_interleaved"))) matrixC[(ROWS_A+1)*COLS_B];
...
void main()
{
  printf("Launching mmul_os opt v2 interleaved memory for dimension %dx%dx%d\n", ROWS_A, COLS_A, COLS_B);
