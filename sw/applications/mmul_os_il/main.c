...
void main()
{
  ...
  if(ROWS_C < CGRA_N_COLS || COLS_C < CGRA_N_ROWS){
    ...
    kcom_perfRecordStart(&(kperf.time.sw));
    //mmulSoftware(outSW);
    cpuMatMul(matrixA, matrixB, outSW, ROWS_A, COLS_A, COLS_B );
    kcom_perfRecordStop(&(kperf.time.sw));
...
void cpuMatMul(int32_t *A, int32_t *B, int32_t *R, unsigned int a_rows, unsigned int a_cols, unsigned int b_cols){
