      ...
      for (int subRowB = 0; subRowB < BLOCK_SIZE; subRowB++){
        ...
        if (subRowB == 0){
          ...
          for(int j = 0; j < CGRA_N_COLS; j++){
            cgra_input[j][cont[j]] = (int32_t) (addrA) + j*COLS_A*4;
            cont[j] += 1;
          }
        } else if (subRowB == 1){
          // Load 4*4*COLS_A
          for(int j = 0; j < CGRA_N_COLS; j++){
            cgra_input[j][cont[j]] = 4*4*COLS_A;
            cont[j] += 1;
          }
        } else if(subRowB == 2){
          cgra_input[1][cont[1]] = numIterations;
          cont[1] += 1;
        }
        int cBAux=cB;
        while(cBAux < cB + CGRA_block_B){
          for(int j = 0; j < CGRA_N_COLS; j++, cBAux++){
            cgra_input[j][cont[j]] = matrixB[(rB+subRowB)*COLS_B+cBAux];
            cont[j] += 1;
          }
        }
      }
