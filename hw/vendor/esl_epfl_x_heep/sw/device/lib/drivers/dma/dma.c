...
dma_config_flags_t dma_load_transaction( dma_trans_t *p_trans )
{
     ...
     * SET THE POINTERS
     */
    dma_cb.peri->SRC_PTR = (uint32_t)(uintptr_t)dma_cb.trans->src->ptr;
    ...
    if(dma_cb.trans->mode != DMA_TRANS_MODE_ADDRESS)
    {
        ...
        */
        dma_cb.peri->DST_PTR = (uint32_t)(uintptr_t)dma_cb.trans->dst->ptr;
     }
     else
     {
        dma_cb.peri->ADDR_PTR = (uint32_t)(uintptr_t)dma_cb.trans->src_addr->ptr;
     }
                                            ...
                                            uint32_t p_inc_du )
{
   ...
   */
    ...
    uint8_t * lasByteInsideRange = p_start + rangeSize -1;
