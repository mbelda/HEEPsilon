...
def plot_speedup_vs_colsB(csv_file, output_image):
    ...
    for colsA in unique_colsA:
        ...
        plt.plot(subset["ColsB"], subset["Speedup"], linestyle='-', label=f"ColsA = {colsA}")

    # Configuración del gráfico
    plt.xlabel("ColsB")
    plt.ylabel("Speedup")
    plt.title("Speedup vs ColsB para distintos ColsA")
    plt.grid(True)
...
def main():
    parser = argparse.ArgumentParser(description="Genera una gráfica de Speedup vs ColsB diferenciada por ColsA.")
